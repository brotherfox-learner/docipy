...
import { FinalExamCongratsOverlay } from "@/components/learn/FinalExamCongratsOverlay";
...
export default function DocumentLearnPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { bundle, loading, error, updateBundle } = useLearningPathBundle(id);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (active) {
          updateBundle({
            documentTitle: bundle?.documentTitle ?? docTitle,
            payload: next,
          });
        }
      } catch {
  ...
  }, [bundle?.documentTitle, docTitle, id, payload?.path, updateBundle]);

  useEffect(() => {
    setLang(payload?.path?.language === "th" ? "th" : "en");
  }, [payload?.path?.language]);
  ...
  async function generate(regenerate: boolean) {
    ...
    try {
      ...
      if (status === 409) {
        setActionError(msg || "Generation already in progress. This page will update automatically.");
        try {
          const lpRes = await api.get(`/api/documents/${id}/learning-path`);
          updateBundle({
            documentTitle: docTitle || "Untitled",
            payload: lpRes.data.data as LearningPathApiPayload,
          });
        } catch {
          /* ignore */
        }
      } else {
        setActionError(msg || "Could not generate learning path.");
      }
    } finally {
      setGenPending(false);
    }
        ...
        <main className="ml-64 flex flex-1 items-center justify-center px-6 py-10">
          <div className="w-full max-w-xl">
            <AiGeneratingPanel
              ...
              description="Fetching lessons, progress, and map data for this document."
            />
          </div>
        </main>
      </div>
  ...
  return (
    <div className="flex min-h-screen w-full overflow-hidden bg-[radial-gradient(circle_at_top,#eef4ff_0%,#f5f7fb_42%,#eef2f8_100%)] font-display dark:bg-[radial-gradient(circle_at_top,#13203f_0%,#09101f_45%,#050914_100%)]">
      <Suspense fallback={null}>
        <FinalExamCongratsOverlay documentId={id} documentTitle={docTitle || "Untitled"} />
      </Suspense>
      <Sidebar />
        ...
        <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="mx-auto w-full max-w-5xl">
                ...
                <div className="grid gap-6 lg:grid-cols-[1fr_0.7fr] lg:items-end">
                  <div>
                    ...
                    <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600 dark:text-slate-300">
                      We will analyze your document and build a guided route with clear lessons, lighter visual
                      explanations, checkpoint quizzes, and a final exam. One generation uses one AI request
            ...
            {isGenerating || genPending ? (
              <AiGeneratingPanel
                title="Building your lessons..."
                description="This can take up to a minute. The page refreshes automatically while your learning structure is being prepared."
                footer={
                  !genPending ? (
                    <button
                      type="button"
                      onClick={() => void generate(true)}
                      className="text-xs font-semibold text-slate-500 underline-offset-2 transition-colors hover:text-primary hover:underline dark:text-slate-400"
                    >
              ...
              <>
                <div className="mb-6 flex flex-wrap items-end justify-end gap-3">
                  <div className="min-w-[180px]">
                    <label
                      htmlFor="learn-regenerate-lang"
                      className="text-[10px] font-bold uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500"
                    >
                      Lesson language
                    </label>
                    <select
                      id="learn-regenerate-lang"
                      value={lang}
                      onChange={(e) => setLang(e.target.value as "en" | "th")}
                  ...
                  <button
                    ...
                    onClick={() => void generate(true)}
                    className="rounded-full border border-slate-200/80 bg-white/80 px-4 py-2.5 text-xs font-bold text-slate-600 transition-colors hover:text-primary disabled:opacity-50 dark:border-white/10 dark:bg-white/5 dark:text-slate-300"
                  >
                    Regenerate path
                  </button>
                </div>
                <LearningMapView documentId={id} documentTitle={docTitle} payload={payload} />
              </>
            ) : null}
          </div>
        </div>
